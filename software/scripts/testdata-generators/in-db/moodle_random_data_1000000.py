...
def publishMoodleUser(cursor, db, userCounter):
...
def publishMoodleForum(cursor, db, forumCounter):
...
def publishMoodleForumPosts(cursor, db, forumPostCounter, forumID, userID, parentForumPost):
...
def produceData(cursor, db):
    ...
    i = 300000
    userCounter = 1
    publishMoodleUser(cursor, db, userCounter)
    forumCounter = 1
    publishMoodleForum(cursor, db, forumCounter) # publish 4000 forums
    forumPostToForum = {}
    publishMoodleForumPosts(cursor, db, i, 1, 3, 0)
    forumID = 1
    userID = 3
    while i <= 1000000:
        if (i %  25000 == 0):
        ...
        if (i % 2500 == 0):
            forumPostToForum = {}
        if (i <= 100000 and i % 2500 == 1 ): # publish continuously during the first 1000000 messages, 40 user
            publishMoodleUser(cursor, db, userCounter)
            userCounter = userCounter + 1
            print ('publish moodle user')
            time.sleep(1 - ((time.time() - starttime) % 1))
        if (i <= 1000000 and i % 250 == 1):
            publishMoodleForum(cursor, db, forumCounter) # publish 400 forums
            forumCounter = forumCounter + 1
            print('publish moodle forum')
            time.sleep(1 - ((time.time() - starttime) % 1))
        forumID = randint(1, forumCounter - 1)
        userID = randint(3, userCounter - 1)
        arentForumPost = randint(0, i)
        if forumID in forumPostToForum:
            parentForumPost = sample(forumPostToForum[forumID], 1)
            forumPostToForum[forumID].append(i)
        else:
            forumPostToForum[forumID] = [0]
        publishMoodleForumPosts(cursor, db, i, forumID, userID, parentForumPost)
        i = i+1
